namespace :symlink do
  ...
  task :create_from do
    ...
    abort "Failed: Provide the path that starts with 'src/guides/v2.3'".red unless path.to_path.start_with? 'src/guides/v2.3'
    abort "Failed: Couldn't find the file at the provided path: #{path}\nTip: Use auto-completion to enter the path.".red unless path.exist?

    new_path = path.sub('/v2.3/', '/v2.4/')

    puts "Creating the #{new_path} symlink from the #{path} file".magenta

    abort "Failed: Couldn't create a symlink, because '#{new_path}' is already there.".red if new_path.exist? || new_path.symlink?
  ...
  task :replace_at do
    ...
    abort "Failed: Provide the path that starts with 'src/guides/v2.4'".red unless path.to_path.start_with? 'src/guides/v2.4'

    new_path = path.sub('/v2.4/', '/v2.3/')

    puts "Replacing the #{new_path} symlink with a copy of the #{path} file".magenta

    abort "Failed: The file '#{path}' is not a symlink or missing".red unless path.exist? and path.symlink?
