    ...
    UISlideform.prototype = {
            ...
            var initSearchFn = function (ev) {
                ...
                if (options == 'email') {
                    if (!classie.has(self.el, 'sb-email-open')) { // open it
                        ...
                        ev.preventDefault();
                        self.open(options);
                        jQuery('#email').fadeIn('fast');
                    }
                    else if (classie.has(self.el, 'sb-email-open') && /^\s*$/.test(self.inputEl.value) && jQuery('#email').attr('placeholder') == '') { // close it
                        ev.preventDefault();
        ...
        close : function(options) {
            ...
            if (options == 'email') {
                ...
                jQuery('#email').attr('placeholder', '');
                if (jQuery('#email').hasClass('ie_check') == false) {
                    jQuery('#email').val('');
                }
                jQuery('#email').fadeOut('slow');
                jQuery('#Emailemail_msg').fadeOut('fast');
            } else {
...
/**
 * Set user agent attribute
 */
var ua = navigator.userAgent,
    doc = document.documentElement;

try {
    var useragent = (ua.match(/MSIE 10.0/i)) ? 'ie10' :
                    (ua.match(/rv:11.0/i))   ? 'ie11' :
                    (ua.match(/Firefox/i))   ? 'ff' :
                    (ua.match(/Chrome/i))    ? 'chrome' :
                    (ua.match(/Opera/i))     ? 'opera' :
