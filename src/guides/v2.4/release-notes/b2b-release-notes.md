...
-  :.fix} Clicking on &quot;Clear All&quot; Button now expands all the filters, rather than collapsing them.
...
-  {:.fix} Previously, approving a Purchase Order with a discount that has expired placed the order at the full amount &amp; not update the Purchase Order total. This now works correctly.
-  {:.fix} A bug was introduced in 2.3.4 where an custom extension attributes would not be copied from the Customer Address to the Quote Address. This bug has been fixed.
...
-  {:.fix} Previously, when attempting to checkout use the WebAPI with a B2B quote, an incorrect value was sent to the API, causing an error to occur. This bug has been fixed.
...
-  {:.fix} Previously, when changing the admin of a company, the original admin address would be copied to the new admin, giving them 2 addresses. Now, only the correct address is added.
