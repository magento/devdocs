...
*  Only whitelisted inline scripts and styles to be compiled by browser
...
For stores that have `unsafe-inline` disabled for `style-src` and `script-src` (default for Magento 2.4) inline scripts and styles
must be whitelisted.
