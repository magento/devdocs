...
Also, the config of the TinyMCE editor builds by `CompositeConfigProvider`
For example:
...
In this case you can extend and fully customize the TinyMCE editor implementing `DefaultConfigProvider` class implements `Magento\Framework\Data\Wysiwyg\ConfigProviderInterfacemethod` in the custom module and implement the method `getConfig` where specify necessary configuration.
