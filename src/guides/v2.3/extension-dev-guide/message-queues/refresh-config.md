...
1.  `Magento\Framework\MessageQueue\PoisonPill\PoisonPillPutInterface` : The interface contains the `put` method. An implementation of the method puts a new version of poison pill inside the database.
1.  `Magento\Framework\MessageQueue\PoisonPill\PoisonPillReadInterface` : The interface contains the `getLatestVersion` method. An implementation of the method gets and returns get latest version of the poison pill.
1.  `Magento\Framework\MessageQueue\PoisonPill\PoisonPillCompareInterface` : The interface contains the `isLatestVersion` method. An implementation of the method checks if the version of current poison pill is the latest.
