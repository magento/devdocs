...
You can use before methods to change the arguments of an observed method by returning a modified argument. If are any arguments, the method should return an array of those arguments. If the method does not change the argument for the observed method, it should return a `null` value.
