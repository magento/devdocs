...
| [cron]| Cron Jobs | none  | `--with-cron`  |  PHP Container, runs cron tasks
...
| [fpm][fpm-container] | PHP FPM | `--php`<br>`--with-xdebug` | 7.0, 7.1, 7.2, 7.3, 7.4 |  Used for all incoming requests. Optionally, add Xdebug to debug PHP code in the Docker environment.
| [fpm_xdebug][fpm_xdebug-container] | Xdebug | `--set-docker-host` | latest | Optional container for PHP debugging<br>On Linux systems, `--set-docker-host` sets the `.host.docker.internal` in the container `/etc/hosts` file.
...
| [tls][tls-container] | SSL Endpoint | `--tls-port` | nginx 1.19-2.0  |  Terminates SSL, can be configured to pass to varnish or nginx
