...
-  `--nginx-worker-processes` to set the number of worker processes for NGINX. Default is `1`.
-  `--nginx-worker-connections` to set maximum number of connections that each worker process can handle simultaneously. The default is set to `1024`.
