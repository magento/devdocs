...
Fastly provides the _Force TLS_ option to redirect unencrypted requests (HTTP) to Fastly. After your Staging or Production environment has been provisioned with a [valid SSL/TSL certificate]({{ site.baseurl }}/cloud/cdn/configure-fastly.html#fastly-tls), you can update the Fastly configuration for your store to enable the Force TLS option.  See the Fastly [Force TLS guide](https://github.com/fastly/fastly-magento2/blob/master/Documentation/Guides/FORCE-TLS.md) in the Fastly CDN Module for Magento 2 documentation.
