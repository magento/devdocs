---
group: cloud-guide
title: Configure Magento split databases solution
functional_areas:
  - Cloud
  - Configuration
  - Setup
...
On the {{site.data.var.ece}} projects, you can use the {{site.data.var.ct}} to configure the [Magento Split Database Performance solution]({{site.baseurl}}/guides/v2.3/config-guide/multi-master/multi-master.html), which offers scalability advantages, including the ability to use three separate master databases for different functional areas of the Magento application.

{:.bs-callout-warning}
If you use the Magento Split database performance solution for your Cloud project, we recommend updating the configuration before you launch your site to production. If you update the configuration on a Production site, switch the site to maintenance mode before applying the split database configuration.

Split DB by separate CLI command

To simplify this procedure on the Cloud, we need to implement an ece-tools db:split CLI command that will fill data for these commands (from $MAGENTO_CLOUD_RELATIONSHIPS ) and run them.

But if it is necessary to make it on production - this procedure should be done with enabled maintenance mode. Considering this, ece-tools db:split command may move project to the maintenance mode if necessary.

To allow possibility for splitting only one type of tables (sales or quotes), this command may have argument type with any of possible values sales and/or quote. By default both types are used

ece-tools db:split --help
