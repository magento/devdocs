...
The configuration repository stores predefined data sets for Magento configuration settings. It is the repository for the [Magento\_Config module]({{ site.mage2100url }}app/code/Magento/Config) and is named `ConfigData.xml`.

Let's see the following example of configuration settings for [Authorize.Net module]({{ site.mage2100url }}app/code/Magento/Authorizenet) `<magento2>/dev/tests/functional/tests/app/Magento/Authorizenet/Test/Repository/ConfigData.xml`.
