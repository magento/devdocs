...
-  **Deployment fixes**
    ...
    -  JIRA-MAGECLOUD-2603-->Now maintenance mode is enabled at the start of the deploy phase and disabled at the end. If something in the deployment fails, the site remains in maintenance mode until deployment issues are resolved. Previously, the site could sometimes resume operation even if the deployment failed, and customers were not aware of configuration or other issues that negatively affect normal site operation.
