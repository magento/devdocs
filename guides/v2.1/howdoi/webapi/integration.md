...
* [Magento REST or SOAP Web APIs]({{ site.gdeurl21 }}get-started/bk-get-started-api.html)
* [Web API authentication]({{ site.gdeurl21 }}get-started/authentication/gs-authentication.html)
* [OAuth-based authentication]( {{ site.gdeurl21 }}get-started/authentication/gs-authentication-oauth.html )
...
2. **Define your module configuration file.** The `etc/module.xml` file provides basic information about the module. Change directories to the `etc` directory and create the `module.xml` file. You must specify values for the following attributes:
   ...
   Module "Magento_Integration" is added to "sequence" to be loaded first. 
...
* Ask for a request token. A request token is a temporary token that the user exchanges for an access token. Use the following API to get a request token from Magento:
  ...
  See [Get a request token]( {{ site.gdeurl21 }}get-started/authentication/gs-authentication-oauth.html#pre-auth-token ) for more details about this call.
...
* Ask for a access token. The request token must be exchanged for an access token. Use the following API to get a request token from Magento:
  ...
  See [Get an access token]( {{ site.gdeurl21 }}get-started/authentication/gs-authentication-oauth.html#get-access-token ) for more details about this call.
...
- [Web API authentication]( {{ site.gdeurl21 }}get-started/authentication/gs-authentication.html)
- [OAuth-based authentication]( {{ site.gdeurl21 }}get-started/authentication/gs-authentication-oauth.html )
- [Magento System Requirements]( {{ site.gdeurl21 }}install-gde/system-requirements.html)
- [Create the module file structure]( {{ site.gdeurl21 }}extension-dev-guide/module-file-structure.html)
- [Create a component]( {{ site.gdeurl21 }}extension-dev-guide/create_component.html)
