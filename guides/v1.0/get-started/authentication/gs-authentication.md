...
<h3>Extension attribute authentication</h3>
...
<b>Question: the following text mentions the `stockItem` attribute. Is this correct, or should it be `stock_item`, as defined in the XML? </b>

In the above linked example, the `stockItem` attribute is restricted to only the users who have the `Magento_CatalogInventory::cataloginventory` permission. As a result, an anonymous or unauthenticated user issuing a `GET http://store/rest/V1/products/` request will receive product information similar to the following:
...
This only works for extension_attributes (e.g. attributes defined in an `extension_attributes.xml` file). There are no permission restrictions on the rest of the returned data. For example, there is no way to restrict `custom_attributes`.
