...
The default Magento `theme.xml` files are located in the following two directories:

* `/app/designfrontend/<vendor_name>/blank`
 
* `/app/designfrontend/<vendor_name>/luma` 

Each `theme.xml` defines the name of that theme as well as the name of the parent theme, if any. The version of the theme is controlled by the <code>composer.json</code> file in which the theme is packaged.
