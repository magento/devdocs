...
User defined EAV attributes can be cached while retrieving them. By default getting all system EAV attributes is cached now. System EAV attributes that should be cached while retrieving are defined in `di.xml` as `attributesForPreload` argument of `Magento\Eav\Model\Config`. But developers can cache custom EAV attributes by running the `bin/magento config:set dev/caching/cache_user_defined_attributes 1` command. This can also be done from the Admin while in Develop mode by setting **Stores** > Settings **Configuration** > **Advanced** > **Developer** > **Caching Settings** > **Cache User Defined Attributes** to **Yes**. Caching EAV attributes while retrieving improves performance as it decreases the amount of insert/select requests to the DB. But it increases cache network size.
