...
`The current user cannot perform operations on cart XXX` | An unauthorized user (guest) tried to add the product into the customer's cart and otherwise, or an authorized user (customer) tried to add the product into the cart of another customer.
`The product's required option(s) weren't entered. Make sure the options are entered and try again.` | Virtual product has customizable options which were not specified in the `mutation` but required for adding product into cart.
