...
In this example, we've supplied a cart ID as input, (which was generated by the [`createEmptyCart` mutation]({{ page.baseurl }}/guides/v2.3/graphql/reference/quote.html)). The output includes the `cart_id` as well as selected information about the items in the cart and the billing and shipping addresses.
