...
<!--- MC-16887-->
*  The `jQuery/ui` library has been refactored into separate widgets for the frontend area so that core modules load only the widgets they need. This update improves the performance of core storefront tasks including the loading of category, configurable product, home, and checkout pages. As a module developer, you must update your code for storefront to remove `jquery/ui` dependency. Otherwise, your clients will see a performance degradation warning message in console.
