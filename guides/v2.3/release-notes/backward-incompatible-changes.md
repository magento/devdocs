...
The `Search`, `CatalogSearch`, `AdvancedSearch`, `Elasticsearch` modules and `Search` framework library were updated to use single interface to resolve a search engine currently set in System Configuration. 

The interface is `\Magento\Framework\Search\EngineResolverInterface`. Its implementation is `\Magento\Search\Model\EngineResolver`. 

This interface provides one method `getCurrentSearchEngine()` that returns current search engine identifier as a string. The implementation has also a fallback mechanism to default `mysql` search engine if current search engine set in the System Configuration doesn't exist. 
