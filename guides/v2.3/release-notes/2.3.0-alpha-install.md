...
In addition to the [standard Magento prerequisites](https://devdocs.magento.com/guides/v2.3/install-gde/prereq/prereq-overview.html), make sure you have obtained and installed:

* *Authentication keys*. If you do not already have authentication keys, contact [Magento Support](https://magento.com/support) for help.
* *PHP 7.1 or 7.2 and extensions*. See instructions here: [Install PHP 7.1 for Ubuntu](https://devdocs.magento.com/guides/v2.3/install-gde/prereq/php-ubuntu.html#instgde-prereq-php71-ubuntu) or [Install PHP 7.1 for Centos](https://devdocs.magento.com/guides/v2.3/install-gde/prereq/php-centos.html#php-centos-71).
...
## Install 2.3.0 Alpha
1. Install the Commerce or Open Source alpha package.
...
3. Install Magento using the CLI or the Web Setup Wizard.
    ...
    **Using the CLI.** This example assumes that the Magento install directory is named `magento2ee`, the `db-host` is on the same machine (`localhost`), and that the `db-name`, `db-user`, and `db-password` are all `magneto`:
    ...
    ```
    For a full description of the CLI install options, refer to [Install the Magento software from the command line](https://devdocs.magento.com/guides/v2.3/install-gde/install/cli/install-cli-install.html#instgde-install-cli-magento).
