...
6.4.1.1. Service contract interfaces SHOULD be placed in separate API modules. All other modules will depend on the API module, while implementations could be easily swapped via `di.xml`. For example, if a module is named `MyModule`, its APIs SHOULD be declared in a module named `MyModuleApi`.

6.4.1.2. Service interfaces which SHOULD be exposed as web APIs MUST be placed under the `MyModuleApi/Api/Service` directory. Service data interfaces MUST be placed under `MyModuleApi/Api/Data`. 
...
6.4.2.1. Methods that have similar MUST serve similar purposes across different services, but they still might have different signatures.

6.4.2.2. Services MUST only operate on scalar types or service data interfaces.

6.4.2.3. Service contracts SHOULD NOT be used for read scenarios on the storefront, GraphQL SHOULD be used for storefront scenarios instead. Check out [web API technical vision]({{ page.baseurl }}/coding-standards/technical-vision/webapi.html) for more details.

6.4.2.4. Each service interface SHOULD declare a single public method. An interface name SHOULD reflect the task/action to be performed. For example, `Magento\InventoryApi\Api\StockSourceLinksDeleteInterface::execute(array $links)`. The only exception is a Repository API which MAY be added for convenience and MUST be limited to singular CRUD operations and `getList($searchCriteria)`.

6.4.3. Service Method Signature

6.4.3.1. Service contracts SHOULD NOT apply presentation layer formatting to the returned data.

6.4.3.2. Service data interfaces represent domain entities and can reference related data interfaces.

6.4.3.3. Service data interfaces MUST NOT contain any business logic. They SHOULD represent a container of data transferable over the wire. All the business logic SHOULD be moved to services.

6.4.3.3. Strict typing is enforced for Service and Data interfaces located under `MyCompany/MyModuleApi/Api`. Only the following types are allowed:
...
* `void` for async operations

6.4.3.4. Service contracts SHOULD support batch data whenever possible. For example, an entity persisting interface SHOULD accept an array of entities to persist instead of a single entity. Customizations via plugins must be adjusted respectively. Batch retrieval operations MUST accept `SearchCriteriaInterface` and return `SearchResultInterface` to support pagination.

6.4.3.5. All services that modify the state MUST support asynchronous execution. Void MUST be used as return type. The status of the operation MAY be checked by polling of the respective query operation. Create operations MUST support UUID generated on the client side.

6.4.3.6. Data objects returned by service contracts SHOULD be fully loaded to ensure consistency.
...
6.4.4.6. If a service method needs to return a modified version of the argument, the original argument object MUST NOT be modified and its copy SHOULD be modified and returned instead.
...
6.4.5.1. Service methods MUST only throw exceptions defined as part of service contract layer. Exceptions thrown from lower layers SHOULD NOT be exposed outside and MUST be wrapped with an appropriate service contract layer exception.
