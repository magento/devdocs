...
File | Description
--- | ---
`.gitignore` | Specify which files and directories to ignore. See [`.gitignore` reference]({{ page.baseurl }}/config-guide/prod/config-reference-gitignore.html){:target="\_blank"}.
...
`/app` | The `code` folder is used for custom modules. The `design` folder is used for custom themes. See [Install a theme]({{ page.baseurl }}/cloud/howtos/custom-theme.html).
`auth.json` | Contains your Magento authorization credentials. See [Prepare your Magento install]({{ page.baseurl }}/cloud/access-acct/first-time-setup_import-prepare.html).
...
`magento-vars.php` | A file used to define multiple stores and sites using [Magento variables]({{ page.baseurl }}/config-guide/multi-site/ms_over.html){:target="\_blank"}.
`/update` | For version 2.2—a service folder used by the support module.
