...
If you declare an observer with a name that is already in use in the same event, Magento will merge these declarations nodes in a single observer declaration respecting the module load order as defined in app/etc/config.php file. This is very useful as you can see in below when disabling an observer declared in another module.
