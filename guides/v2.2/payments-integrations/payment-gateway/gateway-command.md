...
Basic interface for a gateway command is [`\Magento\Payment\Gateway\CommandInterface`]({{ site.mage2200url }}app/code/Magento/Payment/Gateway/CommandInterface.php). It implements the [Command design pattern](http://designpatternsphp.readthedocs.io/en/latest/Behavioral/Command/README.html).
...
(The code sample is from {{site.data.var.ce}} v2.2. Although the payment provider gateway was added in v2.3, the particular default implementation using the gateway were added in v2.1.)
