...
You should also configure the number of threads for input request processing must be configured within the web server's configuation file, as listed below:
...
To guarantee successful execution of all Magento scenarios without dumping data or code to disk, it is recommended to set the memory limit:
...
This document does not attempt to provide in-depth MySQL tuning, as it highly dependent on the store context. We can make some general recommendations.
