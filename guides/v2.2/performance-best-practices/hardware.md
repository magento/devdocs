...
### PHP
Magento 2 has differing PHP memory requirement based on how your system is deployed.  In general, if you are setting up a single server store and plan on using the web setup wizard we recommend configuring PHP memory for 2G.  If you are setting up a site using pipeline deployment we recommend 2G on your build server and 1G on your web nodes.

Scenerios and expected PHP memory requirements:
* Webnode serving only store front pages, 256M
* Webnode serving admin pages with a large catalog, 1G
* Magento 2 cron indexing a site with a large catalog, >256M (please see [advanced-setup](./advanced-setup.md) to tune for optimal performance)
* Magento 2 compile and deploy of static assets, 756M
* Magento 2 using web setup wizard to install or upgrade a store with several 3rd party extensions, 2G
* Magento 2 performance toolkit profile generation, >1G PHP ram, >16M MySQL TMP_TABLE_SIZE & MAX_HEAP_TABLE_SIZE settings
