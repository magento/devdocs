...
* <a id="#mtf_block_types">Create block for the test</a>
  ...
  * <a href="#mtf_block_path">How to determine a block name and a path</a>
    ...
    * <a href="#mtf_block_path_ui">See name and path of blocks in GUI</a>

    * <a href="#mtf_block_path_code">See name and path of blocks in code</a>
...
Magento can show you a block name, path to te PHTML template, and a full class name right on the UI of the Magento page or implicitly in the HTML code of the page.
...
The following is the node that adds the block to the page:
...
|---|---|---|---|---|
|`name`| Name of the block| Required|Name of Magento block . <a id="mtf_block_name_path"> More details about block name and path</a> |`widgetGrid`|
|`class`| Full name of the block class |Required| The name structure is similar to the Magento block class name. <a id="mtf_block_name_path"> More details about block name and path</a>| `Magento\Widget\Test\Block\Adminhtml\Widget\WidgetGrid`|
...
In the MTF you can use mapping for the form in a block and the form in a tab, extending from classes <a href="https://github.com/magento/mtf/blob/develop/Magento/Mtf/Block/Form.php">Magento\Mtf\Block\Form</a>, <a href="https://github.com/magento/magento2/blob/develop/dev/tests/functional/tests/app/Magento/Backend/Test/Block/Widget/FormTabs.php">Magento\Backend\Test\Block\Widget\FormTabs</a>, <a href="https://github.com/magento/magento2/blob/develop/dev/tests/functional/tests/app/Magento/Backend/Test/Block/Widget/Tab.php">Magento\Backend\Test\Block\Widget\Tab</a>.
...
<h3 id="mtf_block_mapping">Form mapping</h3>

Let's see the <a href="https://github.com/magento/magento2/blob/develop/app/code/Magento/Customer/Block/Form/Login.php">Customer Login block</a>. The block has two input fields: `email` and `password`.
...
|`wrapper`|If fields of the form are grouped and have the format `name='group_name[field_name]`, then you can use wrapper that will automatically add `group_name` to the selector of all fields, if applicable.|`login`|
|`fields`| The node containing mapping fields. |`<email>`, `<password />`. |
|`selector`|Value for the selector that is used to find the field. If you use node name same as the name of the field in the block form, than you can omit use of this node.| For `<email>`, the `[name='login[username]']`. For `<password />`, the `[name='login[password]']` implicitly.|
...
You can use form tab mapping, when the form that you want to fill 
is situated on a tab. To get the block class with form tab mapping you should extend from the <a href="https://github.com/magento/magento2/blob/develop/dev/tests/functional/tests/app/Magento/Backend/Test/Block/Widget/FormTabs.php" ><code>Magento\Backend\Test\Block\Widget\FormTabs or Magento\Backend\Test\Block\Widget\Tab</code></a> class.
...
|`selector`| Selector or locator of the tab in the HTML code.|
