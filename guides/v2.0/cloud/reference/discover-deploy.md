...
When you deploy Magento, you push your local branch code to your Git repository. This Git repo is more than a Every active Git repo is part of your project's cluster, with a total of eight active environments.
...
We build only what has changed since the last build. This speeds up deployment in Magento Enterprise Cloud Edition.
...
* Applies patches located in `vendor/magento/magento-cloud-configuration/patches` and any project-specific patches in `m2-hotfixes`
...
At this point, the cluster has not been created yet. So you should not try to connect to a database or imagine anything was daemonized.

When the build completes, the application is mounted on a read-only file system. You will be able to configure specific mount points that are going to be read/write. This means you cannot FTP to the server and add modules. The only way to modify your code, extensions, and ointegrations is through your local Git branch and running `git push`, which builds and deploys the environment.
...
*	Mounts the read-write file system on a highly available distributed storage grid
...
The last step runs a deployment script. You can use this script to anonymize data in development environments, clear caches, ping external continuous integration tools, and so on.

When this script runs, you have access to all the services in your environment including Redis and your database.
