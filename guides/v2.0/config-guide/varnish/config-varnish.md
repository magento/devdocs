...
In the preceding figure, Varnish is *before* the web server, which you can picture inside the Magento box. Varnish proxies HTTP traffic and caches content *before* it reaches the web server.

As a result, Varnish reduces the incoming traffic to Magento.
